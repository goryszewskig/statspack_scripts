    ...
    def plot(self):
        ...
        for fname in os.listdir(self.dirname):
            if fname.endswith("txt") and fname.find(self.name_pattern) >= 0:
                report_file = open(self.dirname + "/" + fname, "r")
                begin_snap = False
                db_time = False
                ...
                for report_line in report_file:
                    try:
                        ...
                        if not report_line_words:
                            continue
                        report_line_long_words = re.split("\s{2,}", report_line)
                        section_line += 1

                        if not begin_snap and report_line.find("Begin Snap:") >= 0:
                        ...
                        elif not db_time and report_line.find("DB Time:") >= 0:
                            snap_data[date]["DB time"] = float(
                                report_line_words[2].replace(",", "")) * 60
                            db_time = True
                        ...
                        elif report_line.startswith("Foreground Wait Events") or \
                                report_line.startswith("Background Wait Events"):
                            wait_class_section = True
                            section_line = 0

                        elif report_line.startswith("Host CPU"):
                            host_cpu_section = True
                            section_line = 0

                        elif host_cpu_section and section_line == 4:
                            snap_data_cpu[date]["Load"] = float(report_line_long_words[4])
                        ...
                        elif load_profile_section and section_line >= 2 and section_line <= 20:
                            ...
                            if load_elem in self.load_profile_elems:
                                if load_elem == "Redo size (bytes)":
                                    snap_data_profile[date][load_elem] = \
                                        round(float(load_val) / 1024 / 1024, 2)
                                else:
                                    snap_data_profile[date][load_elem] = \
                                        float(load_val)

                        elif load_profile_section and report_line.startswith("Instance Efficiency Percentages"):
                            load_profile_section = False

                        elif wait_class_section and len(report_line_long_words) >= 5 \
                                and self.is_float(report_line_long_words[3]):
                            class_name = self.get_class_name(
                                report_line_long_words[0])
                            if class_name not in ("NONE", "Other", "Idle"):
                                if event_class_wait_sum.get(class_name, -1) >= 0:
                                    event_class_wait_sum[class_name] += float(
                                        report_line_long_words[3].replace(",", ""))
                                else:
                                    event_class_wait_sum[class_name] = float(
                                        report_line_long_words[3].replace(",", ""))

                        elif wait_class_section and report_line.startswith("Wait Event Histogram"):
                            wait_class_section = False

                            for class_name in self.event_classes:
                                snap_data[date][class_name] = event_class_wait_sum.get(
                                    class_name, 0)
...
if __name__ == '__main__':
    if len(sys.argv) == 3:
        awr = AWRAnalyzer(sys.argv[1], sys.argv[2])
        awr.plot()
    else:
        print("Original script statspack_analyzer.py by Kamil Stawiarski (@ora600pl)")
        print("Adjusted version by Piotr Wrzosek to visualize data from AWR reports")
        ...
        print("Details can be found on this blogs: blog.ora-600.pl lonedba.wordpress.com ")
        print("and GitHub: https://github.com/ora600pl/statspack_scripts https://github.com/pewu78/statspack_scripts")
